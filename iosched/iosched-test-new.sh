...
test_graph_bonnie()
{
	...
	test_graph_generic "${title}, Sequential Block Writes" "$xlabel" "Writes, K/sec" "${prefix}-seq-write-${graph}" "$data" 1 2
	test_graph_generic "${title}, Sequential Block Re-Writes" "$xlabel" "Re-Writes, K/sec" "${prefix}-seq-rewrite-${graph}" "$data" 1 5
	test_graph_generic "${title}, Sequential Block Reads" "$xlabel" "Reads, K/sec" "${prefix}-seq-read-${graph}" "$data" 1 8

	test_graph_generic "${title}, Sequential Block Write Latency" "$xlabel" "Write latency (ms)" "${prefix}-seq-write-latency-${graph}" "$data" 1 11
	test_graph_generic "${title}, Sequential Block Re-Write Latency" "$xlabel" "Re-Write latency (ms)" "${prefix}-seq-rewrite-latency-${graph}" "$data" 1 14
	test_graph_generic "${title}, Sequential Block Read Latency" "$xlabel" "Read latency (ms)" "${prefix}-seq-read-latency-${graph}" "$data" 1 17
}
...
sched_cfq()
{
	file=/sys/block/${TEST_DRIVE}/queue/scheduler
	echo cfq > $file
}
...
test_run "CFQ" "sched_tweak_cfq_slice_async sched_tweak_cfq_slice_sync" "CFQ:slice_async CFQ:slice_sync" "slice_async slice_sync" "Bonnei++, CFQ" "" test_bonnie $(seq 10 20 150)
